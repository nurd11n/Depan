...
import {
  sourcingQuoteSchema,
  type SourcingQuoteFormInput,
  type SourcingQuoteOutput,
} from "@/lib/schemas";

type Status = "idle" | "submitting" | "success" | "error";

export default function QuoteSourcing() {
  const t = useTranslations("quoteSourcing");
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<SourcingQuoteFormInput, unknown, SourcingQuoteOutput>({
  ...
  async function onSubmit(values: SourcingQuoteOutput) {
  ...
  return (
    <div className="border border-border bg-midnight-light p-9 sm:p-13">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 flex flex-col gap-4">
        ...
        <Field label={t("product")} error={errors.product && tForm("required")}>
          <textarea rows={2} {...register("product")} className={`${inputClass} resize-y`} />
        </Field>
        <Field label={t("targetPrice")}>
          <input type="number" step="0.01" min="0" {...register("targetPricePerUnit")} className={inputClass} />
        </Field>
        <Field label={t("quantity")} error={errors.quantity && tForm("required")}>
          <input type="number" step="1" min="1" {...register("quantity")} className={inputClass} />
        </Field>
        <Field label={t("notes")}>
